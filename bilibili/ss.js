...
(async () => {
	try {
		const response = await got('https://www.bilibili.com/bangumi/play/ss33900',{
      headers: {
    ...
    console.log('success')
    // ss系列
    let str = response.body.match(/window\.\_\_INITIAL\_STATE\_\_\=([\s\S]*?)\;\(function\(\)\{var s\;/)[1];
    // 拿到每一集的cid bv，拿到去请求https://api.bilibili.com/pgc/player/web/playurl?cid=228982363&qn=80&type=&otype=json&fourk=1&bvid=BV1GV411S7B2&ep_id=336506&fnver=0&fnval=80&session=0b15faefbd0bdf25f869b65ac4657f46
    console.log(str)
	} catch (error) {
    console.log('error')
		console.log(error);
...
(async () => {
	try {
		const response = await got('https://api.bilibili.com/pgc/player/web/playurl?cid=228982363&qn=80&type=&otype=json&fourk=1&bvid=BV1GV411S7B2&ep_id=336506&fnver=0&fnval=80&session=0b15faefbd0bdf25f869b65ac4657f46',{
      headers: {
        'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/85.0.4183.83 Safari/537.36',
        'cookie': '_uuid=D87F5597-9F58-9410-7C9A-EC6EE452205A44601infoc; buvid3=69498676-73DD-451C-A606-02CA33B5AEE553917infoc; CURRENT_FNVAL=16; LIVE_BUVID=AUTO2115924816109117; Hm_lvt_8a6e55dbd2870f0f5bc9194cddf32a02=1596722341; CURRENT_QUALITY=116; blackside_state=1; finger=481832271; sid=ax362991; DedeUserID=14899614; DedeUserID__ckMd5=6fb380360cc351f0; SESSDATA=122d1191%2C1614940578%2C4f586*91',
        'referer': 'https://www.bilibili.com/bangumi/play/ss33900'
      }
    });
    console.log('success')
    // ss系列 获取下载地址
